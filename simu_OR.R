...
simu_OR <- function(alter_comp=1, pc=1, f0, f1, trainsition_mat, n_test, n_node, size_train){
  # alter_comp = 1, f1 <- c(mean, sd)
  ...
  nf_f <- function(x){
    ...
    denom = 0
    if(alter_comp == 1){
      denom = dnorm(x,f1[1],f1[2])
    } else {
      for(i in 1:length(pc)){
        denom = denom + dnorm(x,f1[i,1],f1[i,2]) * pc[i]
      }  
    }
  ...
  if(alter_comp == 1){
    ...
    s_test = dt$s
    o_test = dt$o
    em.res<-em.hmm(o_test, alter_comp, maxiter=500)
  }
