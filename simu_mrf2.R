...
genData <- function(crfmodel, f0, pc, f1){
  status <- as.vector(sample.chain(crfmodel, 1))-1
  dt <- rep(0,crfmodel$n.nodes)
  for(i in 1:crfmodel$n.nodes){
    if(status[i] == 0){
      dt[i] <- rnorm(1,f0[1],f0[2])
    } else {
      j <- sample(1:length(pc),size=1,prob=pc)
      dt[i] <- rnorm(1,f1[j,1],f1[j,2])
    }
  }
  return(list(s=status,o=dt))
}



simu_mrf2 <- function(mc, pc=1, f0, f1, trainsition_mat, n_test, n_node, size_train, j){
  ...
  dt_test = genData(mc, f0, pc, f1)  
  s_test = dt_test$s
  o_test = dt_test$o
  em.res<-em1.hmm(o_test, maxiter=500)
  ...
  dt = cbind(as.vector(o_train), as.vector(s_train)-1, as.vector(o_test), as.vector(s_test))
  write.table(dt, paste('~/Documents/Paper3/Simu/RNN/data/',j,'.csv',sep = ''), 
